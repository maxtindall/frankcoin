...
async function main() {
  ...
  // Prefer the on-chain IDL (permissionless: any forked miner can fetch it),
  // but fall back to the IDL bundled next to this script. The on-chain IDL
  // account only exists if `anchor idl init` was run at deploy time; without
  // this fallback the miner dies with "could not fetch IDL" even though the
  // program is deployed and mineable.
  let idl = await anchor.Program.fetchIdl(PROGRAM_ID, provider).catch(() => null);
  if (!idl) {
    const here = path.dirname(fileURLToPath(import.meta.url));
    const local = [
      path.join(here, 'frankcoin.idl.json'),
      path.join(here, '..', 'target', 'idl', 'frankcoin.json'),
    ].find(p => fs.existsSync(p));
    if (!local) throw new Error('no on-chain IDL and no bundled IDL found (frankcoin.idl.json)');
